package dsls.regex
import RegularExpressionImplicits._
...
//  val telNumber = Concat(areaCode, Concat(threeDigits, Concat(Literal('-'), fourDigits)))
   val telNumber = '(' ~ digit.repeat(3) ~ ')' ~ digit.repeat(3) ~ '-' ~ digit.repeat(4)
